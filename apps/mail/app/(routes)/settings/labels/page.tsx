	...
	return (
		<div className="grid gap-6">
			<SettingsCard
				...
				action={
					<T id="_routes_.settings.labels.page.5">
						<Form {...form}>
							<form>
										...
										<DialogContent showOverlay={true}>
											<DialogHeader>
												<DialogTitle>
													<Var>
														{editingLabel ? (
															<T id="_routes_.settings.labels.page.1">
																{"Edit Label"}
															</T>
														) : (
															<T id="_routes_.settings.labels.page.2">
																{"Create New Label"}
															</T>
														)}
													</Var>
											...
											<div className="space-y-4 py-4">
												<div className="space-y-2">
													<FormField
														...
														name="name"
														render={({ field }) => (
															<T id="_routes_.settings.labels.page.6">
																<FormItem>
											...
											<div className="flex justify-end space-x-2">
												...
												<Button
													className="h-8 [&_svg]:size-4"
													onClick={form.handleSubmit(onSubmit)}
												>
													<Var>
														{editingLabel ? (
															<T id="_routes_.settings.labels.page.3">
																{"Save Changes"}
															</T>
														) : (
															<T id="_routes_.settings.labels.page.4">
																{"Create Label"}
															</T>
														)}
													</Var>
					...
					<ScrollArea className="h-full pr-4">
						<div className="space-y-4">
							{isLoading && !error ? (
								...
								<T id="_routes_.settings.labels.page.0">
									<p className="text-muted-foreground py-4 text-center text-sm">
								...
								<div className="grid grid-cols-2 gap-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-7">
									{labels?.map((label) => {
										return (
											<T id="_routes_.settings.labels.page.7">
												<div
													key={label.id}

...
function LoginClientContent({ providers, isProd }: LoginClientProps) {
	...
	const displayProviders = isProd
		? providers.filter((p) => p.enabled || p.isCustom)
		: providers;
	...
	return (
		<T id="_auth_.login.login_client.2">
					...
					<div className="w-full space-y-4">
						...
						<Var>
							{shouldShowDetailedConfig && (
								<T id="_auth_.login.login_client.0">
										...
										<div className="flex flex-col space-y-4">
											...
											<p className="text-sm text-black/80 dark:text-white/80">
												...
												<span className="font-semibold">
													<Var>{missingRequiredProviders.join(", ")}</Var>
												</span>
											...
											<div className="space-y-3">
												<Var>
													{missingProviders.map((provider) => (
														<T id="_auth_.login.login_client.3">
															<div
																key={provider.id}
															...
															>
																...
																>
																	<div className="flex items-center">
																		<Var>{getProviderIcon(provider.id)}</Var>
																		...
																		<Var>
																			{provider.envVarStatus.map((envVar) => (
																				<div
																					...
																					style={{
																						transitionDelay: expandedProviders[
																							provider.id
																						]
																				...
																				>
																					...
																					<div className="flex-1">
																						<div className="flex items-center">
																							...
																							>
																								{!envVar.set ? (
																									<T id="_auth_.login.login_client.4">
																										{"Missing"}
																									</T>
																								) : (
																									<T id="_auth_.login.login_client.5">
																										{"Set"}
																									</T>
																								)}
																							</span>
						...
						<Var>
							{shouldShowSimplifiedMessage && (
								<T id="_auth_.login.login_client.1">
									...
									{sortedProviders.map(
										(provider) =>
											(provider.enabled || provider.isCustom) && (
												<T id="_auth_.login.login_client.6">
													<Button
														key={provider.id}
													...
													>
														<Var>{getProviderIcon(provider.id)}</Var>
														Continue with <Var>{provider.name}</Var>
													</Button>
												</T>
											),
									)}
...
export function LoginClient(props: LoginClientProps) {
	const fallback = (
		<T id="_auth_.login.login_client.7">

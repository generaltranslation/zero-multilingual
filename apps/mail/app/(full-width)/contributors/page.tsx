...
const ChartControls = ({
	...
	<T id="_full_width_.contributors.page.4">
		<div className="mb-4 flex items-center justify-between">
			<span className="text-muted-foreground text-sm">
				Showing{" "}
				<Var>
					{showAll ? (
						<T id="_full_width_.contributors.page.0">{"all"}</T>
					) : (
						<T id="_full_width_.contributors.page.1">{"top 10"}</T>
					)}
				</Var>{" "}
				contributors
			</span>
			...
			>
				...
				<Var>
					{showAll ? (
						<T id="_full_width_.contributors.page.2">{"less"}</T>
					) : (
						<T id="_full_width_.contributors.page.3">{"all"}</T>
					)}
				</Var>{" "}
				(<Var>{total}</Var>)
			</Button>
...
export default function OpenPage() {
			...
			?.filter(
				(contributor) =>
					...
					coreTeamMembers.some(
						(member) =>
							member.toLowerCase() === contributor.login.toLowerCase(),
					),
			)
			.sort((a, b) => {
	...
	if (error) {
		return (
			<T id="_full_width_.contributors.page.5">
	...
	if (isLoading) {
		return (
			<T id="_full_width_.contributors.page.6">
	...
	return (
		<T id="_full_width_.contributors.page.7">
										...
										<Tooltip
											content={({ active, payload }) => {
												if (active && payload && payload.length) {
													return (
														<T id="_full_width_.contributors.page.8">
									...
									>
										...
										<Tooltip
											content={({ active, payload }) => {
												if (active && payload && payload.length) {
													return (
														<T id="_full_width_.contributors.page.9">
															<div className="rounded-lg border border-neutral-200 bg-white p-3 shadow-lg dark:border-neutral-800 dark:bg-neutral-900">
																<div className="grid gap-2">
																	<div className="flex items-center gap-2">
					...
					<div className="mb-12 space-y-6">
						<div className="text-center">
							...
							<p className="text-muted-foreground mt-2">
								Meet the people behind 0.email
							</p>
						</div>
										...
										<div className="flex-1">
											...
											<div className="mt-3 flex gap-2">
												...
												{specialRoles[member.login.toLowerCase()]?.website && (
													<Link
														href={
															specialRoles[member.login.toLowerCase()]
																?.website || "#"
														}
														target="_blank"
														className="rounded-md p-1 text-neutral-600 transition-colors hover:bg-neutral-100 hover:text-neutral-900 dark:text-neutral-400 dark:hover:bg-neutral-800 dark:hover:text-white"
					...
					<div className="mb-16 space-y-6">
						<div className="text-center">
							...
							<div className="text-muted-foreground mt-2 flex items-center justify-center gap-2">
								<span>
									Thank you to all the contributors who have helped make 0.email
									possible
								</span>
							</div>
						</div>
												...
												<Tooltip
													...
													content={({ active, payload }) => {
														if (active && payload && payload.length) {
															...
															return (
																<T id="_full_width_.contributors.page.10">
																	<div className="rounded-lg border border-neutral-200 bg-white p-3 shadow-lg dark:border-neutral-800 dark:bg-neutral-900">
																		<div className="flex items-center gap-2">
								...
								<div className="flex flex-col items-center gap-8 md:flex-row">
									<div className="w-full md:w-2/3">
										...
										<p className="mt-3 text-neutral-600 dark:text-neutral-300">
											Whether you&apos;re fixing bugs, adding features, or
											...
											<div className="flex items-center gap-4">
												...
												<div className="text-sm text-neutral-600 dark:text-neutral-300">
													<span className="font-semibold text-neutral-900 dark:text-white">
														<Var>{filteredContributors?.length || 0}</Var>
													</span>{" "}
													contributors
												</div>
											</div>

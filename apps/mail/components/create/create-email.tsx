...
export function CreateEmail({
	...
	return (
		<>
			<T id="components.create.create_email.1">
				<Dialog open={!!isComposeOpen} onOpenChange={handleDialogClose}>
					<div className="flex min-h-screen flex-col items-center justify-center gap-1">
						<div className="flex w-[750px] justify-start">
							<DialogClose asChild className="flex">
								<button className="flex items-center gap-1 rounded-lg bg-[#F0F0F0] px-2 py-1.5 dark:bg-[#1A1A1A]">
									...
									<span className="text-sm font-medium text-[#6D6D6D] dark:text-white">
										esc
									</span>
								</button>
							</DialogClose>
						...
						<Var>
							{isDraftLoading ? (
								<T id="components.create.create_email.0">
								...
								<EmailComposer
									...
									initialTo={
										typedDraft?.to?.map((e: string) =>
											e.replace(/[<>]/g, ""),
										) || processInitialEmails(initialTo)
									}
									initialCc={
										typedDraft?.cc?.map((e: string) =>
											e.replace(/[<>]/g, ""),
										) || processInitialEmails(initialCc)
									}
									initialBcc={
										typedDraft?.bcc?.map((e: string) =>
											e.replace(/[<>]/g, ""),

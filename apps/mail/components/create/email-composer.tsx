...
export function EmailComposer({
	...
	const { mutateAsync: aiCompose } = useMutation(
		trpc.ai.compose.mutationOptions(),
	);
	const { mutateAsync: createDraft } = useMutation(
		trpc.drafts.create.mutationOptions(),
	);
	...
	useEffect(() => {
		...
		if (!emailData?.latest || !mode || !session?.activeConnection?.email)
			return;
	...
	return (
		<T id="components.create.email_composer.4">
			<div
				className={cn(
			...
			>
				<div className="border-b border-[#E7E7E7] pb-2 dark:border-[#252525]">
					...
					>
						...
						<Var>
							{showCc && (
								<T id="components.create.email_composer.0">
						...
						<Var>
							{showBcc && (
								<T id="components.create.email_composer.1">
									<div className="flex items-center gap-2 px-3">
				...
				<div className="relative -bottom-1 flex flex-col items-start justify-start gap-2 self-stretch border-t bg-[#FFFFFF] px-3 py-3 outline-white/5 dark:bg-[#202020]">
					...
					>
						<EditorContent
							editor={editor}
							className="prose dark:prose-invert prose-headings:font-title focus:outline-none max-w-full"
						/>
					</div>
							...
							<div className="flex items-center justify-start gap-2">
								...
								<Var>
									{attachments && attachments.length > 0 && (
										<T id="components.create.email_composer.3">
											<Popover modal={true}>
															...
															<div className="flex items-center justify-between">
																...
																<Var>
																	{attachments && attachments.length > 1 && (
																		<T id="components.create.email_composer.2">
																			<button
																				type="button"
																				onClick={(e) => {
																					e.preventDefault();
							...
							<Tooltip>
								<TooltipTrigger asChild>
									...
									>
										<Var>
											{messageLength < 50 && (
												<ShortStack className="h-3 w-3 fill-[#9A9A9A]" />
											)}
										</Var>
										<Var>
										...
										<Var>
											{messageLength >= 200 && (
												<LongStack className="h-3 w-3 fill-[#9A9A9A]" />
											)}
										</Var>
									</button>
								</TooltipTrigger>
...
const ContentPreview = ({
	...
	<T id="components.create.email_composer.5">
		<motion.div

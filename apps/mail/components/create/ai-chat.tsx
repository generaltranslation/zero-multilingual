...
export function AIChat() {
	...
	const { messages, input, setInput, error, handleSubmit, status, stop } =
		useChat({
			...
			<div className="flex-1 overflow-y-auto" ref={messagesContainerRef}>
				<div className="min-h-full space-y-4 px-4 py-4">
					{chatMessages && !chatMessages.enabled ? (
						<T id="components.create.ai_chat.0">
						...
						<T id="components.create.ai_chat.1">
							<div className="absolute inset-0 flex flex-col items-center justify-center">
								...
								>
									...
									{toolParts.map((part, idx) =>
										...
										"threads" in part.toolInvocation.result ? (
											...
											<T id="components.create.ai_chat.4">
												<span className="text-muted-foreground flex gap-1 text-xs">
													<CheckCircle2 className="h-4 w-4" />
													Used tool: <Var>{part.toolInvocation.toolName}</Var>
												</span>
											</T>
										) : null,
									)}
					...
					{status === "submitted" && (
						<T id="components.create.ai_chat.2">
					...
					{(status === "error" || !!error) && (
						<T id="components.create.ai_chat.3">

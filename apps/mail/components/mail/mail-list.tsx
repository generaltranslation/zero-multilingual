	...
	({
		...
		useEffect(() => {
			if (getThreadData?.latest?.tags) {
				setIsStarred(
					getThreadData.latest.tags.some((tag) => tag.name === "STARRED"),
				);
			}
		...
		const handleNext = useCallback(
			(id: string) => {
				if (!id || !threads.length || focusedIndex === null)
								...
								<div className="w-full">
									<div className="flex w-full flex-row items-center justify-between">
										<div className="flex flex-row items-center gap-1">
											...
											>
												<span
													className={cn(threadId ? "max-w-[3ch] truncate" : "")}
												>
													{highlightText(
										...
										{latestMessage.receivedOn ? (
											...
											>
												{formatDate(
													latestMessage.receivedOn.split(".")[0] || "",
												)}
											</p>
										) : null}
									</div>
								...
								<Avatar className="h-8 w-8 rounded-full border dark:border-none">
									<div
										...
										onClick={(e: React.MouseEvent) => {
											...
											setMail((prev: Config) => ({
												...prev,
												bulkSelected: prev.bulkSelected.filter(
								...
								<div className="w-full">
										...
										<div className="flex flex-row items-center gap-[4px]">
											...
											>
												{isFolderSent ? (
													<span
														className={cn(
															"truncate text-sm md:max-w-[15ch] xl:max-w-[25ch]",
														)}
													>
														{highlightText(
															latestMessage.subject,
									...
									<div className="flex justify-between">
										{isFolderSent ? (
											...
											<p
												className={cn(
													"mt-1 line-clamp-1 w-full min-w-0 text-sm text-[#8C8C8C]",
												)}
											>
												{highlightText(
													latestMessage.subject,
...
export function MailListDemo({
	...
	onSelectMail,
}: { items?: typeof items; onSelectMail?: (message: any) => void }) {
	return (
					...
					{filteredItems.map((item) => {
						return item ? (
							<Thread
								...
								selectMode={"single"}
								onClick={(message) => () =>
									onSelectMail && onSelectMail(message)
								}
...
export const MailList = memo(({ isCompact }: MailListProps) => {
	...
	const [{ refetch, isLoading, isFetching, hasNextPage }, items, , loadMore] =
		useThreads();

	const allCategories = Categories();

	// Skip category filtering for drafts, spam, sent, archive, and bin pages
	const shouldFilter = !["draft", "spam", "sent", "archive", "bin"].includes(
		folder || "",
	);
			...
			>
				...
				>
					{isLoading ? (
						...
						<T id="components.mail.mail_list.0">
								...
								.map((data, index) => {
									...
									return isFolderDraft ? (
										...
										<Thread
											...
											key={`${data.id}-${index}`}
											isKeyboardFocused={
												focusedIndex === index && keyboardActive
											}
											isInQuickActionMode={
												isQuickActionMode && focusedIndex === index
											}

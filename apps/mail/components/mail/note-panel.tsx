...
function SortableNote({
	...
	return (
		<T id="components.mail.note_panel.0">
			<div
				ref={setNodeRef}
				style={style}
				className={cn(
						...
						<Tooltip>
							<TooltipTrigger asChild>
								<div className="mt-2 flex cursor-default items-center text-xs text-[#8C8C8C]">
									...
									<span>
										<Var>{formatRelativeTime(note.createdAt, format)}</Var>
									</span>
								</div>
							</TooltipTrigger>
							<TooltipContent
							...
							>
								<Var>
									{note.updatedAt > note.createdAt ? (
										<>
											<p>
												{t("common.notes.created")}:{" "}
												{formatDate(note.createdAt, format)}
											</p>
											<p>
												{t("common.notes.updated")}:{" "}
												{formatDate(note.updatedAt, format)}
											</p>
										</>
									) : (
										<p>
					...
					<div className="flex items-center">
						...
						<DropdownMenu>
							<DropdownMenuTrigger asChild>
								<Button
									variant="ghost"
									className="h-8 w-8 p-0 opacity-30 group-hover:opacity-100"
								>
							...
							>
								...
								>
									...
									<span>
										<Var>{t("common.notes.actions.edit")}</Var>
									</span>
								</DropdownMenuItem>
								<DropdownMenuItem
									onClick={onCopy}
								...
								>
									...
									<span>
										<Var>{t("common.notes.actions.copy")}</Var>
									</span>
								</DropdownMenuItem>
								<DropdownMenuItem
									onClick={onTogglePin}
								...
								<DropdownMenuSub>
									<DropdownMenuSubTrigger className="text-black focus:bg-white focus:text-black dark:text-white/90 dark:focus:bg-[#202020] dark:focus:text-white">
										...
										<span>
											<Var>{t("common.notes.actions.changeColor")}</Var>
										</span>
									</DropdownMenuSubTrigger>
									<DropdownMenuPortal>
								...
								>
									...
									<span>
										<Var>{t("common.notes.actions.delete")}</Var>
									</span>
								</DropdownMenuItem>
							</DropdownMenuContent>
						</DropdownMenu>
...
export function NotesPanel({ threadId }: NotesPanelProps) {
	...
	const handleDragEnd = async (event: DragEndEvent) => {
		...
		if (over && active.id !== over.id) {
			...
			if (
				!activeNote ||
				!overNote ||
				activeNote.isPinned !== overNote.isPinned
			) {
				setActiveId(null);
				return;
			}
			...
			if (activeNote.isPinned) {
				...
				const reorderedPinnedNotes =
					assignOrdersAfterPinnedReorder(newPinnedNotes);
	...
	const sortedPinnedNotes = useMemo(
		() => sortNotesByOrder(pinnedNotes),
		[pinnedNotes],
	);

	const sortedUnpinnedNotes = useMemo(
		() => sortNotesByOrder(unpinnedNotes),
		[unpinnedNotes],
	);

	const pinnedIds = useMemo(
		() => sortedPinnedNotes.map((note) => note.id),
		[sortedPinnedNotes],
	);
										...
										<>
											{searchQuery && filteredNotes.length === 0 ? (
												<div className="flex flex-col items-center justify-center py-6 text-center">
													...
													<p className="text-sm text-black dark:text-white/90">
														{t("common.notes.noMatchingNotes", {
															query: searchQuery,
														})}
													</p>
													<Button
											...
											{isAddingNewNote && (
												<T id="components.mail.note_panel.2">
													<div className="relative mb-3 overflow-hidden rounded-md border border-[#E7E7E7] bg-[#FFFFFF] p-3 dark:border-[#252525] dark:bg-[#202020]">
														<div
															...
															style={
																selectedColor !== "default"
																	? getNoteColorStyle(selectedColor)
																	: {}
															}
														/>
																	...
																	<kbd className="bg-muted inline-flex items-center rounded border px-1.5 py-0.5 font-mono text-[10px]">
																		<Var>
																			{navigator.platform.includes("Mac") ? (
																				...
																				<T id="components.mail.note_panel.1">
																					{"Ctrl"}
																				</T>
																			)}
																		</Var>

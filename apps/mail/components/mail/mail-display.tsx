...
const StreamingText = ({ text }: { text: string }) => {
	...
	return (
		<div className="flex items-center gap-2">
			...
			>
				{isThinking ? (
					<T id="components.mail.mail_display.0">
...
const AiSummary = () => {
	...
	return (
		<T id="components.mail.mail_display.1">
			<div
			...
			>
				...
				>
					<TextShimmer className="text-xs font-medium text-[#929292]">
						Summary
					</TextShimmer>
				...
				<Var>
					{showSummary && (
						<Markdown markdownContainerStyles={{ fontSize: 12 }}>
							{summary?.data.short || ""}
						</Markdown>
					)}
				</Var>
...
const MailDisplay = ({ emailData, index, totalEmails, demo }: Props) => {
	...
	const renderPerson = useCallback(
		(person: Sender) => (
			<T id="components.mail.mail_display.2">
					...
					<PopoverTrigger asChild>
						...
						>
							...
							<div className="justify-start text-sm font-medium leading-none text-[#1A1A1A] dark:text-white">
								<Var>{person.name || person.email}</Var>
							</div>
						</div>
					</PopoverTrigger>
					<PopoverContent className="text-sm">
						<p>
							Email: <Var>{person.email}</Var>
						</p>
						<p>
							Name: <Var>{person.name || "Unknown"}</Var>
						</p>
					</PopoverContent>
				</Popover>
			</T>
	...
	return (
		<T id="components.mail.mail_display.4">
			<div
							...
							{index === 0 && (
								<>
									<span className="inline-flex items-center gap-2 font-medium text-black dark:text-white">
										...
										{emailData?.tags ? (
											<MailDisplayLabels
												labels={emailData?.tags.map((t) => t.name) || []}
											/>
										) : null}
									</span>
															...
															<Tooltip>
																<TooltipTrigger asChild>
																	<span className="text-sm">
																		+{people.length - 2}{" "}
																		{people.length - 2 === 1 ? (
																			<T id="components.mail.mail_display.5">
																				{"other"}
																			</T>
																		) : (
																			<T id="components.mail.mail_display.6">
																				{"others"}
																			</T>
																		)}
																	</span>
										...
										<div className="flex w-full flex-col">
											<div className="flex w-full items-center justify-between">
															...
															<div className="space-y-1 text-sm">
																<div className="flex">
																	<span className="w-24 text-end text-gray-500">
																		<Var>{t("common.mailDisplay.from")}</Var>:
																	</span>
																	<div className="ml-3">
																...
																<div className="flex">
																	<span className="w-24 text-end text-gray-500">
																		<Var>{t("common.mailDisplay.to")}</Var>:
																	</span>
																...
																<div className="flex">
																	<span className="w-24 text-end text-gray-500">
																		<Var>{t("common.mailDisplay.date")}</Var>:
																	</span>
																	<span className="text-muted-foreground ml-3">
																		<Var>
																			{format(
																				new Date(emailData?.receivedOn),
																				"PPpp",
																			)}
																		</Var>
																	</span>
																	...
																	{emailData.tls && (
																		<div className="flex items-center">
																			...
																			<div className="text-muted-foreground ml-3 flex items-center gap-1">
																				...
																				{t(
																					"common.mailDisplay.standardEncryption",
																				)}
																			</div>
																		</div>
																	)}
												...
												<time className="text-sm font-medium text-[#6D6D6D] dark:text-[#8C8C8C]">
													<Var>{formatDate(emailData?.receivedOn)}</Var>
												</time>
											</div>
											<div className="flex items-center gap-1 pb-2">
													...
													<Var>
														{(() => {
															...
															if (
																...
																return (
																	<T id="components.mail.mail_display.7">
															...
															const visibleRecipients = allRecipients.slice(
																0,
																3,
															);
												...
												<Var>
													{(emailData?.bcc?.length || 0) > 0 && (
														<T id="components.mail.mail_display.3">
															<p className="text-sm font-medium text-[#6D6D6D] dark:text-[#8C8C8C]">
																Bcc:{" "}
																<Var>

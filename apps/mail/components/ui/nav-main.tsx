...
export function NavMain({ items }: NavMainProps) {
	...
	const handleFilterByLabel = (label: LabelType) => () => {
		...
		const newValue = existingValue
			? `${existingValue} label:${label.name}`
			: `label:${label.name}`;
		setSearchValue({
			value: newValue,
			highlight: "",
	...
	return (
		<SidebarGroup
			className={`${state !== "collapsed" ? "" : "mt-1"} space-y-2.5 py-0`}
		>
			<SidebarMenu>
				{items.map((section) => (
					<Collapsible
											...
											<Var>
												{(() => {
													...
													const groupedLabels = data.reduce(
														(acc, label) => {
															const isFolderLabel = /[^/]+\/[^/]+/.test(
																label.name,
															);
															if (isFolderLabel) {
																const [groupName] = label.name.split("/") as [
																	string,
																];
																if (!acc[groupName]) {
																	acc[groupName] = [];
...
function NavItem(item: NavItemProps & { href: string }) {
	...
	if (item.disabled) {
		return (
			<SidebarMenuButton
				tooltip={
					state === "collapsed" ? t(item.title as MessageKey) : undefined
				}
				className="flex cursor-not-allowed items-center opacity-50"
			>
				{item.icon && (
					<item.icon ref={iconRef} className="relative mr-2.5 h-3 w-3.5" />
				)}
	...
	const buttonContent = (
		<SidebarMenuButton
			...
			className={cn(
				...
				item.isActive &&
					"bg-subtleWhite text-accent-foreground dark:bg-[#202020]",
			)}
			onClick={() => setOpenMobile(false)}
		>
			{item.icon && <item.icon ref={iconRef} className="mr-2 shrink-0" />}
			<p className="mt-0.5 min-w-0 flex-1 truncate text-[13px]">
				{t(item.title as MessageKey)}
			</p>
			{stats &&
				item.id?.toLowerCase() !== "sent" &&
				stats.some(
					(stat) => stat.label?.toLowerCase() === item.id?.toLowerCase(),
				) && (
